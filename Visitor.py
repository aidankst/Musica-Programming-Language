...
class MusicaException(Exception):
...
class Visitor(MusicaVisitor):
    ...
    def visitLookpartitura(self, ctx):
        if not(len(self.parti)): print("No notes in the sheet music")
        else:
            a = str(self.parti)
            a = a.replace(",","")
            a = a.replace('"','')
    ...
    def visitProc(self,ctx):
        ...
        if name in self.procs:
            ...
            raise MusicaException('Proc \"' + name + '\" undefined.')
    ...
    def visitProcDef (self,ctx):
        ...
        parametros = self.visit(children[1])
        if name in self.procs:
            raise MusicaException('Proc \"' + name + '\" already defined.')
    ...
    def visitDiv(self,ctx):
        ...
        den = self.visit(l[2])
        if den == 0:
            raise MusicaException('Division by zero.')
    ...
    def visitTall(self, ctx):
        ...
        size = len(self.stack[-1][ctx.VAR().getText()])
        if i < 1 or i > (size):
            raise MusicaException('indice ' + str(i) + ' does not belong to the list ' + ctx.VAR().getText())
        else:
